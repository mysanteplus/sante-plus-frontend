...
export async function checkAndDisplaySubscriptionStatus() {
    try {
        ...
        if (status) {
            ...
            localStorage.setItem("user_type_compte", status.type_compte || "AVEC_PATIENT");
        }
...
/**
 * 🎨 AFFICHER LE BADGE D'ABONNEMENT DANS LE HEADER
 */
export function renderSubscriptionBadge() {
    ...
    const badgeContainer = document.getElementById("subscription-badge-container");
    ...
    if (!isActive) {
        badgeContainer.innerHTML = `
            <button onclick="window.switchView('subscription')" 
                    class="px-3 py-1.5 bg-amber-100 text-amber-700 rounded-xl text-[9px] font-black uppercase hover:bg-amber-200 transition-all flex items-center gap-1">
                ⚠️ Inactif
            </button>
        `;
        return;
    ...
    badgeContainer.innerHTML = `
        <div class="flex items-center gap-2">
            <span class="px-3 py-1.5 ${statusColor} rounded-xl text-[9px] font-black uppercase flex items-center gap-1">
                ${statusText}
            </span>
            ${endDateFormatted && daysRemaining > 0 ? `
...
window.markAsPaid = async (id, montant) => {
    ...
    try {
        ...
        // ✅ METTRE À JOUR LE BADGE APRÈS PAIEMENT
